...
type Kafka struct {
	Start     bool
	OrderSaga OrderSagaKafka
}

type OrderSagaKafka struct {
	Hosts []string
	Topic string
}
...
func getKafka(v *viper.Viper) (Kafka, error) {
	const (
		startKey = "KAFKA_START_KEY"
	)
	...
	if !v.IsSet(startKey) {
		return kafka, fmt.Errorf("%w: %s", ErrMissingRequiredConfig, startKey)
	}

	if kafka.OrderSaga, err = getOrderSagaKafka(v); err != nil {
		return kafka, err
	}
...
func getOrderSagaKafka(v *viper.Viper) (OrderSagaKafka, error) {
	const (
		hostsKey            = "KAFKA_ORDER_SAGA_HOSTS"
		orderToSagaTopicKey = "KAFKA_ORDER_TO_SAGA_TOPIC"
	)

	var orderSagaKafka OrderSagaKafka

	if !v.IsSet(hostsKey) {
		return orderSagaKafka, fmt.Errorf("%w: %s", ErrMissingRequiredConfig, hostsKey)
	}

	if !v.IsSet(orderToSagaTopicKey) {
		return orderSagaKafka, fmt.Errorf("%w: %s", ErrMissingRequiredConfig, orderToSagaTopicKey)
	}

	orderSagaKafka.Hosts = v.GetStringSlice(hostsKey)
	orderSagaKafka.Topic = v.GetString(orderToSagaTopicKey)

	return orderSagaKafka, nil
}
